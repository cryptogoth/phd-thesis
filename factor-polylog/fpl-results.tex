...
seem to
be more powerful than \textsf{1D NTC}. However, use of these gates does not result in an
asymptotic improvement over \textsf{1D NTC}.
...
Our approach is the first implementation for factoring on \textsf{2D NTC},
augmented with a classical controller and parallel, communicating
modules (\textsf{2D CCNTCM}).
%
\begin{table}[htb!]
\begin{center}
