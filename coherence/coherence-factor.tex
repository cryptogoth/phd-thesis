...
Config Log Depth ($d$) & $O(\frac{1}{d}\log n)$ & $O(\frac{1}{2^d}n^{{2^d}+2})$ \\
Log Depth ($d = \lceil \log_2 n \rceil$)  & $O(\log^{1} n)$ & $O(n^{n+2})$ \\
...
%depth of $O(\log n)$. Instead, one can consider a configurable-depth
%QPF where $n/2^{d-1}$ integers are multiplied in parallel in depth $O(d)$,
%and there are $2^{d-1}$ such groups multiplied in serial, giving a total depth of $O(2^d \log n)$,
