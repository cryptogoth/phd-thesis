...
(called a \emph{fanout rail}). The result of a fanout is an $n$-qubit cat
state stored in the fanout rail, as shown in the following equation.
...
\alpha\ket{0}^{\otimes n+1} + \beta\ket{1}^{\otimes n+1} \rightarrow^{unfanout}
...
interleaved layers of $CNOT$s in a sense ``disentangle'' the qubits from
