...
We must further make an assumption that we have chosen
a ``reasonable'' partitioning of $W$ qubits into $\overline{W}$ modules,
each of size $\hat{W}$. This means that at most $\hat{W}$ 
 where reasonableness is defined below in
}
Section \ref{subsec:mod-size}. In fact, we are able to partition our
hybrid factoring architectures in this dissertation into such reasonable
modules.
...
Note that \textsf{CCNTCM} can be defined for other dimensionalities,
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Old stuff, to be re-edited later
