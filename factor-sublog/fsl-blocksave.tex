...
\begin{equation}
n_k = n + \sum_{l=1}^k \log_2^{(l)}n = O(n)
\end{equation}
...
of $2 \times n$-qubit numbers in $O(\log^{*}n)$ depth on CCNTC.
\end{theorem}
...
steps on $t'\log_2^{(k)}$ input numbers of size $n + \sum_{l=1}^k \log_2^{(l)}n$. 
...
$\log_2^{(k+1)}t'$ bits (of different significance). This leaves 
...
With the quantum compiling overhead discussed in Section \ref{fsl-qcompile},
we can now 